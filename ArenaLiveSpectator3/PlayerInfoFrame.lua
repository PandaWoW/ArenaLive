		...
		if ( name and specID ) then
			...
			for tier = 1, NUM_TALENTS do
				...
				if ( talentID ) then
					...
					button.talentID = nil;
					button.name:SetText(L["Not Selected"]);
			...
			for slot = 1, NUM_GLYPHS do
				...
				local glyphID = ArenaLiveSpectator.UnitCache:GetUnitSelectedGlyphBySlot(self.unit, glyphOrder[slot]);
				if ( glyphID ) then
