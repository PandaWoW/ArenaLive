...
ArenaLiveSpectator.UnitCache = CreateFrame("Frame", "ArenaLiveSpectatorUnitCache");
	...
	for tier = 1, MAX_TALENT_TIERS do
		for column = 1, NUM_TALENT_COLUMNS do
			...
			if ( talentID and selected ) then
				...
				--ArenaLive:Message("     Tier %d selected Talent is %s (%d)", "debug", tier, name, talentID);
			end
		end
	end
...
function UnitCache:OnUpdate(elapsed)
	if ( self.enabled ) then
		...
		if ( ELAPSED >= ON_UPDATE_THROTTLE ) then
			...
			-- up to date.
			if ( not ArenaLiveSpectator:HasMatchStarted() and theTime >= NEXT_PLAYER_INFO_REFRESH ) then
