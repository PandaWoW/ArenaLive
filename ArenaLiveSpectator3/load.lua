...
ArenaLiveSpectator.defaults = {
	...
	["TeamA"] = {
		...
		["Leader"] = "",
		["Score"] = 0,
		["Colour"] = { 0, 0.5, 1, 1 }, -- Blue Team (old gold: 1, 0.83, 0, 1)
	},
	["TeamB"] = {
		["Name"] = "Red Team",
		["Leader"] = "",
		["Score"] = 0,
		["Colour"] = { 1, 0.19, 0, 1 }, -- Red Team (old green:  0.03, 0.71, 0, 1)
	},
	["MatchStatistic"] = {
	},
	["NicknameDatabase"] = {
...
function ArenaLiveSpectator:Enable(battlefieldStatusID)
	...
	-- Set Smart Camera Mode:
	local database = ArenaLive:GetDBComponent(addonName);
	C_Commentator.SetUseSmartCamera(database.UseBlizzSmartCam);
	C_Commentator.SetSmartCameraLocked(database.UseBlizzSmartCam);	
...
function ArenaLiveSpectator:RequestLeader()
	if ( not IsInGroup() or UnitIsGroupLeader("player") ) then
		...
		if ( battleTag ) then
			local _, _, _, _, gameAccountID = ArenaLiveSpectatorWarGameMenu:GetPlayerDataByBattleTag(battleTag);
			if ( gameAccountID ) then
				local _, toonName, client, toonRealm = BNGetGameAccountInfo(gameAccountID);
...
function ArenaLiveSpectator:OnEvent(event, ...)
	...
	elseif ( event == "AL_SPEC_PLAYER_UPDATE" ) then
		...
		self.SmartCamera:UpdateUnit(unit);
...
function ArenaLiveSpectator:UpdateDB()
	...
	if ( database.Version == "3.2.3b" ) then
		database.Version = "3.2.6b";
	end
	
	if ( database.Version == "3.2.6b") then
		database.TeamB.Colour = { 0.39, 0.71, 0, 1 }; -- Green team's colour will be more saturated this way.
		database.Version = "3.2.7b";
	end
	
	if ( database.Version == "3.2.7b") then
		database.TeamA.Colour = { 0, 0.5, 1, 1 };
		database.TeamB.Colour = { 1, 0.19, 0, 1 };
		database.UseBlizzSmartCam = true;
		database.Version = "3.2.8b";
	end
	
	if ( database.Version == "3.2.8b" ) then
		database.Version = "3.2.9b";
	end
end
...
ArenaLiveSpectator:RegisterEvent("BN_FRIEND_INFO_CHANGED");
